...
- 新增严格的 `fabric migrate analyze/apply` 源码迁移器：`analyze` 支持本地源码、`npm:<spec>` 和 `file:<tgz>` 的无脚本 intake，区分 native admission、portable/manual/blocked/source-missing/not-dsh-plugin；仅自动转换纯 `shell.overlay` → `fabric.hud` 子集，绝不兼容执行已发布 DSH bundle。
